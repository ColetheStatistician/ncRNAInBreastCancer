...
# Prep data
formattedHCP <- formatHCPs(covariates = covariates, expression_matrix = Y_nc)
Z <- formattedHCP$Z
Y <- formattedHCP$Y

# Fix parameters
GENOTYPE_file_name = genotype; EXP_file_name = exp_nc; gene_location_file_name = geneloc; snps_location_file_name =snploc
k=10; lambda1=10; lambda2=2; lambda3=2; first_two_plus_k_pcs=2

# Get eQTLs
eQTLs_nc <- MeQTLandHcp(Z=Z, Y=Y, GENOTYPE_file_name=GENOTYPE_file_name, gene_location_file_name=gene_location_file_name, snps_location_file_name=snps_location_file_name, EXP_file_name=EXP_file_name,
...
saveRDS(end_statistics, "nc_prelim_results.rds") 
